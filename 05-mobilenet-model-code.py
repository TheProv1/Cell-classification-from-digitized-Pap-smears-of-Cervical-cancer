...
# --- HARD GPU SELECTION: Make only physical GPU 1 visible to TensorFlow ---
os.environ['CUDA_VISIBLE_DEVICES'] = '1'

# --- FIX: Add this before importing TensorFlow to prevent potential cuDNN errors ---
...
# --- OPTIMIZATION: Enable Mixed Precision ---
keras.mixed_precision.set_global_policy('mixed_float16')

# --- 1. Global Configuration ---
DATASET_DIR = "/home/Tojo/papsmear"
TRAIN_DIR = os.path.join(DATASET_DIR, "training")
MODEL_SAVE_PATH = "final-mobilenet-tfrecord.keras" # Updated model name

# --- TFRecord File Paths (assumes they are in the current directory) ---
...
IMAGE_SIZE = (224, 224)
BATCH_SIZE = 64 # Increased for V100 performance
EPOCHS = 50
SEED = 123
...
# --- 2. GPU Initialization (Simplified) ---
print("--- Configuring GPU for MobileNet Training ---")
...
# --- 3. Data Loading for Metadata (Class Weights & Test Labels) ---
# We no longer load all image paths, just the labels for evaluation and class counts.
...
try:
    ...
    print("❌ Error: train.csv or test.csv not found. Please ensure they are in the current directory.")
    exit()

# --- 4. Class Weight Calculation ---
print("\n--- Calculating Class Weights from Training Split ---")
...
def parse_tfrecord_fn(example):
    """Parses a single TFRecord example, decodes the image, and one-hot encodes the label."""
    feature_description = {
    ...
    label = tf.one_hot(example['label'], depth=NUM_CLASSES) # One-hot encode the label
    return image, label

def configure_dataset(tfrecord_path, shuffle=False):
    """Configures a tf.data pipeline for TFRecords for maximum performance."""
    ...
    if shuffle:
        ds = ds.shuffle(buffer_size=2048, seed=SEED) # A larger buffer is better for large datasets
        
    ds = ds.batch(BATCH_SIZE)
    ds = ds.prefetch(buffer_size=AUTOTUNE) # Prefetch batches for the GPU
    return ds
...
with tf.device('/gpu:0'):
    ...
    base_model = keras.applications.MobileNetV3Small(include_top=False, weights="imagenet", input_shape=(*IMAGE_SIZE, 3))
...
print("\n✅ Confusion matrix saved to mobilenet_confusion_matrix.png")