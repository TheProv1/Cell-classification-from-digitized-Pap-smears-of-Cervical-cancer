...
DATASET_DIR = "/home/Tojo/papsmear" # Make sure this path is correct
TRAIN_DIR = os.path.join(DATASET_DIR, "training") # Used to get class names
TEST_TFRECORD = 'test.tfrecord'
TEST_LABELS_CSV = 'test.csv'

BATCH_SIZE = 128  # Use a larger batch size for faster evaluation on V100
AUTOTUNE = tf.data.AUTOTUNE
TARGET_GPU = '/gpu:0' # TensorFlow will now see GPU 1 as /gpu:0

# --- VRAM-Optimized Model Order ---
MODEL_FILES = [
...
def configure_dataset(tfrecord_path, image_size, num_classes):
    ...
    # Define the parsing function *inside* this function to capture the arguments
    def parse_tfrecord_fn(example):
        ...
        image = tf.image.resize(image, image_size) # Use the provided image_size
...
def evaluate_model(model_path, test_dataset, true_labels, class_names_list):
    ...
    with open(log_filename, 'w') as f, redirect_stdout(f):
        try:
            ...
            # model.predict can handle datasets that yield (images, labels), it just ignores the labels.
...
if __name__ == "__main__":
    for model_file in MODEL_FILES:
        ...
        current_image_size = (224, 224) # Default size
        ...
        # Create a new, correctly-sized dataset for the current model
        test_ds = configure_dataset(TEST_TFRECORD, image_size=current_image_size, num_classes=NUM_CLASSES)
        
        # Evaluate the model with its corresponding dataset
