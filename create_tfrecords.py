# create_tfrecords.py (Final Version: Parallel CPU Processing, Iterative Writing)
import tensorflow as tf
import numpy as np
import os
from tqdm import tqdm

# --- Configure GPU Memory Growth (Best Practice) ---
# Prevents TensorFlow from allocating all VRAM at startup.
...
def write_tfrecords_parallel(csv_path, output_path):
    ...
    # Force all operations to the CPU to avoid GPU bottlenecks
    with tf.device('/cpu:0'):
        ...
        # This will read and serialize images in the background using all CPU cores.
        ...
        # This is the robust method that keeps memory usage low.
        with tf.io.TFRecordWriter(output_path) as writer:
            # Wrap the dataset with tqdm to create a live progress bar
            with tqdm(total=len(paths), unit="records", desc=f"Writing {os.path.basename(output_path)}") as pbar:
                # The for loop pulls one processed record at a time from the parallel pipeline
                for record in dataset:
                    # The record is an EagerTensor; .numpy() gets its raw byte value
...
# --- Execute the Process ---
# Process all three data splits using the robust parallel method.
